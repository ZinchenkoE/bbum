...
//            ],
            ...
            [
                ['parent_id'], 'integer',
                'message' => 'Неверный формат id родительской категории',
            ],
        ];
    }
    ...
    protected function get($key)
    {
        ...
        if ($key == 'new') {
            ...
            $this->grest->data['category'] = $this->db->createCommand("SELECT * FROM bs_category WHERE category_id = {$key}")->queryOne();
            $this->grest->render           = 'category/action-category';
        } else {
    ...
    protected function create()
    {
        ...
        if ($this->validate()) {
            $key = $this->db->createCommand("SELECT AUTO_INCREMENT FROM information_schema.tables WHERE TABLE_NAME = 'bs_category';")->queryScalar();
            ...
            $this->grest->setCode(400, 'Данных не могут быть добавлены');
        }
    }
    ...
    {
        $r = $this->db->createCommand("DELETE FROM bs_category WHERE category_id=:id;")->bindValue(':id', $key)->execute();
        if ($r) {
